...
return ( <div className="min-h-screen bg-gray-100 p-4 text-sm font-sans"> {!user ? ( <div className="max-w-md mx-auto bg-white p-6 rounded shadow"> <h2 className="text-2xl text-center font-bold mb-4">Hotel Login</h2> <input className="w-full p-2 mb-3 border rounded" type="email" placeholder="Email" value={email} onChange={(e) => setEmail(e.target.value)} /> <input className="w-full p-2 mb-3 border rounded" type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} /> <button
onClick={handleLogin}
className="w-full bg-blue-600 text-white py-2 rounded mb-2"
>Login</button> <button
onClick={handleSignup}
className="w-full bg-gray-600 text-white py-2 rounded"
>Sign Up</button> </div> ) : ( <div className="max-w-5xl mx-auto"> <div className="bg-white p-4 rounded shadow mb-4 flex justify-between items-center"> <h2 className="text-lg font-semibold">Welcome, {user.email}</h2> <button
...
<div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
      ...
      <div className="bg-white p-4 rounded shadow mb-6">
        ...
        <div className="grid sm:grid-cols-2 gap-4">
          <input className="border p-2 rounded" placeholder="Guest Name"
            value={newBooking.guestName}
            onChange={(e) => setNewBooking({ ...newBooking, guestName: e.target.value })} />
          <select className="border p-2 rounded" value={newBooking.roomId}
          ...
          <input className="border p-2 rounded" type="number" placeholder="Guests"
            value={newBooking.guests}
            onChange={(e) => setNewBooking({ ...newBooking, guests: +e.target.value })} />
          <input className="border p-2 rounded" type="number" placeholder="Amount (₹)"
            value={newBooking.amount}
            onChange={(e) => setNewBooking({ ...newBooking, amount: +e.target.value })} />
          <input className="border p-2 rounded" type="number" placeholder="Advance (₹)"
            value={newBooking.advance}
            onChange={(e) => setNewBooking({ ...newBooking, advance: +e.target.value })} />
        </div>
        <button onClick={handleBooking} className="mt-4 bg-blue-600 text-white py-2 px-6 rounded">Book</button>
      </div>

      <div className="bg-white p-4 rounded shadow">
        <h3 className="text-lg font-semibold mb-4">Booking History ({today})</h3>
          ...
          <tbody>
            {bookings.map((b) => (
              <tr key={b.id} className="text-center border-t">
                ...
                <td>₹{b.advance}</td>
                <td>
                  <button onClick={() => downloadInvoice(b)} className="text-blue-600 underline">Invoice</button>
                  {user.uid === ADMIN_UID && (
                    <button onClick={() => handleDelete(b.id)} className="text-red-600 ml-2 underline">Delete</button>
                  )}
                </td>
              </tr>
            ))}
